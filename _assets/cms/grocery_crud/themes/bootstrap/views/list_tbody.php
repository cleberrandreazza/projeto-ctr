<?php foreach($list as $num_row => $row){ ?>
    <tr>
        ...
        <?php }?>
        <td <?php if ($unset_delete) { ?> style="border-left: none;"<?php } ?> class="al-center">
                <div class="only-desktops"  style="white-space: nowrap">
                    <?php if (!empty($row->action_urls) || !$unset_read || !$unset_delete) { ?>
                    <div class="btn-group dropdown">
                    <?php if(!$unset_edit){?>
                        <a class="btn btn-success" href="<?php echo $row->edit_url?>"><span class="glyphicon glyphicon-pencil"></span></a>
                    <?php } ?>
                        <?php
                        ...
                        ?>
                        <?php if (!$unset_read) { ?>
                                <a href="<?php echo $row->read_url?>" class="btn btn-default"><span class="glyphicon glyphicon-eye-open"></span></a>
                        <?php } ?>
                        <?php if (!$unset_delete) { ?>
                                <a data-target="<?php echo $row->delete_url?>" href="javascript:void(0)" title="<?php echo $this->l('list_delete')?>" class="delete-row btn btn-danger">
